...
## Input Files
- `memory/reward-signals.jsonl` — Conversation excerpts with potential rewards
...
### 1. Read the signals
```bash
cat ~/.openclaw/workspace/memory/reward-signals.jsonl
```

### 2. Read current state
```bash
cat ~/.openclaw/workspace/memory/reward-state.json
```
...
```bash
~/.openclaw/workspace/skills/vta-memory/scripts/anticipate.sh --add "thing to look forward to"
```

If an anticipated thing happened, resolve it:
```bash
~/.openclaw/workspace/skills/vta-memory/scripts/resolve-anticipation.sh --item "thing that happened" --reward
```

### 6. Sync State File
```bash
~/.openclaw/workspace/skills/vta-memory/scripts/sync-motivation.sh
```

### 7. Update Watermark

```bash
~/.openclaw/workspace/skills/vta-memory/scripts/update-watermark.sh --from-signals
```
